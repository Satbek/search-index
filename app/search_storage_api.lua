...
function M.user_id.get_by_name_birthdate(hash, data)
    return get_users_by_hash(hash, data, M.cmp_data.name_birthdate)
end

function M.user_id.add_name_birthdate_identifier(user_id, hash, data, bucket_id)
    local ok, err = pcall(add_identifier, user_id, hash, data, bucket_id)
    if not ok then
        return identifier_err:new('add_name_birthdate_identifier: ' .. err)
    end
    return ok
end

function M.user_id.delete_name_birthdate_identifier(user_id, hash)
    local ok, err = pcall(delete_identifier, user_id, hash)
    if not ok then
        return identifier_err:new('delete_name_birthdate_identifier: ' .. err)
    end
    return ok
end
